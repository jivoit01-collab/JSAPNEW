...
async function openEmpTasksModal(empId, empName) {
    ...
    try {
        ...
        let html = tasks.map(function (t) {
            ...
            const statusColors = { 'completed': '#10b981', 'overdue': '#ef4444', 'pending': '#f59e0b', 'submitted': '#f59e0b', 'in-progress': '#3b82f6', 'not-started': '#94a3b8' };
...
function renderHODTable(tasks) {
    ...
    let rows = tasks.map(function (t, index) {
        ...
        const isCompleted = !!getValue(t, 'isCompleted', 'IsCompleted');
        const overdue = !isCompleted && new Date(expectedEndDate) < new Date();
        const statusColors = { 'COMPLETED': '#10b981', 'overdue': '#ef4444', 'PENDING': '#f59e0b', 'SUBMITTED': '#f59e0b', 'IN_PROGRESS': '#3b82f6' };
...
function fmtStatus(s) { return { PENDING: 'Pending', SUBMITTED: 'Submitted', IN_PROGRESS: 'In Progress', COMPLETED: 'Completed', NOT_STARTED: 'Not Started' }[s] || s || 'Pending'; }
